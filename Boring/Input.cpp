...
vec2 Input::mouse = vec2();
double Input::scroll = 0;
Input::Input(GLFWwindow* win, int WIDTH, int HEIGHT) {
	this->win = win;
	Input::mouse = vec2();
...
vec2 Input::getJoystickAxis() {
	int present = glfwJoystickPresent(GLFW_JOYSTICK_1);
	int count;
	const float* name = glfwGetJoystickAxes(GLFW_JOYSTICK_1, &count);
	return vec2(name[0], -name[1]);
}
vec2 Input::getMouse() {
	return vec2(Input::mouse.x - WIDTH / 2, Input::mouse.y - HEIGHT / 2);
}
